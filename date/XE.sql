...
create table tbl_middle_menu(
    ...
    constraint tbl_top_menu_menu_no foreign key(menu_no) references tbl_top_menu(menu_no) ON DELETE CASCADE
);
...
create table tbl_bottom_menu(
    ...
    constraint tbl_middle_menu_menu_no foreign key(menu_no) references tbl_middle_menu(menu_no) ON DELETE CASCADE
);
